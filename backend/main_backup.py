...
import uuid
try:
    # Prefer centralized DB setup
    from database.connection import init_db as _db_init
except Exception:
    _db_init = None  # type: ignore
import requests
import time
import csv
...
# Import price repository for SQLite caching
...
async def startup_event():
    ...
    if _app_started:
        ...
        return
    # Initialize SQLite database if SQLAlchemy is available
    try:
        if _db_init is not None:
            _db_init()
        else:
            print("⚠️ DB init function unavailable; skipping")
    except Exception as e:
    ...
    def init_cache():
        try:
            ...
            if os.getenv("SKIP_CACHE_PREPOPULATION", "0") != "1":
            ...
            # Always refresh interesting losers
...
_OVERSOLD_SCAN_CACHE_TTL_SECONDS = int(os.getenv("OVERSOLD_SCAN_CACHE_TTL_SECONDS", "300"))
...
def _oversold_scan(symbols: List[str]) -> List[dict]:
    ...
    def worker(sym: str):
        try:
            # Use function with Polygon fallback
...
def premium_oversold_scan(req: OversoldScanRequest):
    ...
    if hit and (now_ts - hit[0]) < _OVERSOLD_SCAN_CACHE_TTL_SECONDS:
        ...
        ranked = _oversold_scan(symbols)
        _OVERSOLD_SCAN_CACHE[key] = (now_ts, ranked)
    return {"candidates": ranked[: req.top], "total": len(ranked), "cached": hit is not None and (now_ts - hit[0]) < _OVERSOLD_SCAN_CACHE_TTL_SECONDS}
...
def premium_oversold_promote(req: PromoteRequest, db: Session = Depends(get_db_session)):
    ...
    for it in candidates:
        ...
        s = TradingSignal(
            ...
            confidence_score=min(1.0, abs(float(score))) if isinstance(score, (int, float)) else 0.5,
            oversold_score=float(score) if score is not None else 0.0,
            news_score=None,
            ml_prediction=None,
            triggering_event_id=None,
            analysis_summary="Rule-based oversold promotion",
            features_json=_json.dumps(m),
            status="active",
...
def premium_oversold_news(symbol: str, db: Session = Depends(get_db_session)):
    ...
    # Lightweight structured classification prompt
    prompt = (
        "Classify the following headlines for {sym}. Return STRICT JSON with keys: "
        "event_type, severity in [minor,moderate,severe], time_horizon in [days,quarters,permanent], "
        "company_specific (bool), has_numbers (bool), credibility in [low,medium,high], novelty_score in [0,1].\n\n"
    ).format(sym=symbol)
    items = [f"- {h.title} ({h.url})" for h in headlines]
    prompt += "\n" + "\n".join(items[:8])
    raw = _call_openai(prompt)
...
# --- Scheduled scanner (lightweight, every 15 min) ---
_OVERSOLD_SCHEDULE_ENABLED = os.getenv("OVERSOLD_SCHEDULE_ENABLED", "1") not in {"0", "false", "False"}
_OVERSOLD_UNIVERSE = (os.getenv("OVERSOLD_UNIVERSE", "AAPL TSLA NVDA AMD AMZN MSFT META GOOGL NFLX JPM BA").split())
...
def _run_oversold_scheduled_scan():
    try:
        ...
        if now.weekday() >= 5:
            return
        # Compute
        ranked = _oversold_scan(_OVERSOLD_UNIVERSE)
...
def _start_oversold_scheduler():
    if not _OVERSOLD_SCHEDULE_ENABLED:
...
def _fetch_daily_history_prefer_stooq(symbol: str) -> List[dict]:
    ...
    # Check if we have this symbol in the SQLite cache
    cached_rows = _extract_stock_from_grouped_cache(symbol)
    if cached_rows and len(cached_rows) >= 20:  # Need at least 20 days for proper metrics
        print(f"Using SQLite cache for {symbol} ({len(cached_rows)} days)")
        return cached_rows
    elif cached_rows:
        print(f"SQLite cache has only {len(cached_rows)} days for {symbol}, need 20+ - trying Polygon API")
...
def _prepopulate_polygon_cache():
